...
- Webhook event bodies now include `external_id` — the value passed at invoice creation, or `null` when not supplied. Use it to correlate webhooks back to your own orders without parsing FintraPay's internal `invoice_id`. Verifier itself unchanged (signs opaque bytes); see `webhook.go` example for usage.
